...
struct fio_option fio_options[FIO_MAX_OPTS] = {
	...
	{
		.name = "targ_dir",
		.lname = "Target Directory",
		.type = FIO_OPT_STR_STORE,
		.off1 = offsetof(struct thread_options, targ_dir),
		.help = "Target directory to find blocks for",
		.category = FIO_OPT_C_FILE,
		.group = FIO_OPT_G_FILENAME,
	},
	{
	...
	},
	{
		...
		.posval = {
			...
			{.ival = "randtrimwrite", .oval = TD_DDIR_RANDTRIMWRITE, .help = "Randomly trim and write mix, trims preceding writes"},
		},
	},
	{
		.name = "rw_sequencer",
...
#error "FIO_HAVE_IOPRIO_CLASS requires FIO_HAVE_IOPRIO"
	...
	},
	{
		...
		.posval = {
			...
			{.ival = "both", .oval = IO_LOG_SAMPLE_BOTH, .help = "Log both average and maximum values over the window"},
			/* Compatibility with former boolean values */
			{
				.ival = "0",
				.oval = IO_LOG_SAMPLE_AVG,
